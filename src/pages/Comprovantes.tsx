...
import { Plus, Receipt, Download, Eye, Trash2, Search, Upload, FileText, Image, Calendar, HelpCircle, X } from "lucide-react";
...
export default function Comprovantes() {
  ...
  const handleBatchUpload = async () => {
    if (batchFiles.length === 0 || !batchMetadata.tipo || !batchMetadata.data_pagamento) {
      toast({
        title: "Erro",
        description: "Selecione os arquivos e preencha todos os campos obrigatórios",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < batchFiles.length; i++) {
        ...
        try {
          // Comprimir imagem (mesmo processo do novo comprovante)
          const compressedBlob = await compressImage(file);
          const fileExt = file.name.split('.').pop();
          const fileName = `${user.id}/${Date.now()}_${i}.${fileExt}`;

          // Upload para storage (mesmo processo do novo comprovante)
          const { error: uploadError } = await supabase.storage
            .from('comprovantes')
            .upload(fileName, compressedBlob);
          ...
          // Inserir no banco (mesmo formato do novo comprovante)
          const { error: insertError } = await supabase
            .from('comprovante')
            .insert([{
              tipo: batchMetadata.tipo,
              arquivo: file.name,
              arquivo_url: fileName,
              data_pagamento: batchMetadata.data_pagamento,
              id_lancamento: null,
              id_usuario: userData.id_usuario
            }]);
      ...
      toast({
        ...
        description: `${successCount} de ${totalFiles} arquivo(s) enviado(s) com sucesso`,
      });
      ...
      setBatchFiles([]);
      setBatchMetadata({ tipo: "", data_pagamento: "" });
      setBatchProgress(0);
    } catch (error) {
      console.error('Erro no upload em lote:', error);
      toast({
        title: "Erro",
        description: "Não foi possível completar o upload em lote",
        variant: "destructive",
      });
    } finally {
          ...
          <Dialog open={isBatchUploadOpen} onOpenChange={setIsBatchUploadOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Envie múltiplos comprovantes de uma vez. Todos terão os mesmos metadados.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div>
                  ...
                  {batchFiles.length > 0 && (
                    <div className="mt-3 space-y-2">
                      <p className="text-sm font-medium">{batchFiles.length} arquivo(s) selecionado(s)</p>
                ...
                <div>
                  <Label htmlFor="batch-tipo">Tipo de Comprovante *</Label>
                  <Select 
                    value={batchMetadata.tipo} 
                    onValueChange={(value) => setBatchMetadata({...batchMetadata, tipo: value})}
                  >
                    <SelectTrigger>
                ...
                <div>
                  <Label htmlFor="batch-data">Data do Pagamento *</Label>
                  <Input
                    id="batch-data"
                    type="date"
                    value={batchMetadata.data_pagamento}
                    onChange={(e) => setBatchMetadata({...batchMetadata, data_pagamento: e.target.value})}
                  />
                </div>
                {batchUploading && (
                ...
                <Button 
                  ...
                  className="w-full"
                >
                  {batchUploading ? "Enviando..." : `Enviar ${batchFiles.length} arquivo(s)`}
                </Button>
              </div>
            </DialogContent>
          </Dialog>
