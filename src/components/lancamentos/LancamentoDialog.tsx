...
export function LancamentoDialog({ onSuccess, lancamento }: LancamentoDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Limpar espaços acidentais antes de enviar
      const descricao = formData.descricao.trim();
      const categoria = formData.categoria.trim();
      const valor = parseFloat(formData.valor);

      // Verificação simples para evitar salvar campos "vazios"
      if (!descricao) {
        toast({
          title: "Erro",
          description: "A descrição não pode estar vazia ou conter apenas espaços.",
          variant: "destructive",
        });
        setLoading(false);
        return;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(e) => setFormData({ ...formData, descricao: e.target.value.trimStart() })}
              required
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => setFormData({ ...formData, categoria: e.target.value.trimStart() })}
              required
            />
          </div>
